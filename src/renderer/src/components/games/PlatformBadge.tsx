...
const PLATFORM_CONFIG: Record<Platform, { label: string; bg: string; text: string }> = {
  ...
  gog: { label: 'GOG', bg: 'bg-[#2c1a4a]', text: 'text-[#a855f7]' },
  ...
  emulator: { label: 'Emulador', bg: 'bg-[#3d2e0a]', text: 'text-[#f59e0b]' },
  other: { label: 'Otro', bg: 'bg-[#1e293b]', text: 'text-[#94a3b8]' }
}
...
export default function PlatformBadge({ platform }: { platform: Platform }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider ${config.bg} ${config.text}`}
    >
      {config.label}
    </span>
  )
